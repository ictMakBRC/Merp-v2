...
class HrSupervisorDashboard extends Component
{
    public function averageTatForContractRenewal(){

        $supervisedEmployees=myTeam(auth()->user()->employee->id);
        return OfficialContract::whereIn('employee_id',$supervisedEmployees)
        ->groupby('employee_id')
    ...
    public function joinedLeftPerYear(){
        ...
        $employeeData = Employee::whereIn('employee_id',$supervisedEmployees)->selectRaw('YEAR(join_date) as year, COUNT(*) as join_count')
            ->groupBy('year')
            ->get();

        $leftEmployeeData = Employee::whereIn('employee_id',$supervisedEmployees)->selectRaw('YEAR(left_at) as year, COUNT(*) as left_count')
    ...
    public function averageContractsSalary(){
        $supervisedEmployees=myTeam(auth()->user()->employee->id);
        return OfficialContract::whereIn('employee_id',$supervisedEmployees)->get()
        ->groupby('employee_id')
    ...
    {
        return view('livewire.human-resource.dashboard.hr-supervisor-dashboard',$this->getData());
    }
}
