    ...
    public function savePayment($id)
    {
        ...
        if ($invoice) {
            ...
            DB::transaction(function () use ($invoice, $newTotalPaid, $status) {
                ...
                // dd($this->ledgerCur);
                $current_balance = $ledgerAccount->current_balance + $this->ledgerIncome;
                $ledgerAccount->current_balance = $current_balance;
                $ledgerAccount->save();
        ...
        if ($invoiceData) {
            ...
            if($invoiceData->invoice_type =='External'){                
                ...
                if($invoiceData->billed_department){
                    $this->budgetLines = FmsBudgetLine::with('budget')->WhereHas('budget', function ($query) use($invoiceData) {
                    ...
                    $this->from_account = $this->ledger->id;
                ...
                $this->baseAmount = $this->amount * $this->rate;
            }
