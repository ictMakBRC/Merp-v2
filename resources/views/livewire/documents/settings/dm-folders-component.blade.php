<div>
                        ...
                        <div class="col-sm-12 mt-3">
                            <div class="d-sm-flex align-items-center">
                                <h5 class="mb-2 mb-sm-0">
                                    @if (!$toggleForm)
                                        Document Folders (<span class="text-danger fw-bold">{{ $folders->total() }}</span>)
                                ...
                                <tbody>
                                    @foreach ($folders as $key => $folder)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $folder->name }}</td>
                                            <td>{{$folder->parent->name??'N/A' }}</td>
                                            <td>{{ $folder->description ?? 'N/A' }}</td>
                                            @if ($folder->is_active == 0)
                                                <td><span class="badge bg-danger">Suspended</span></td>
                                            @else
                                                <td><span class="badge bg-success">Active</span></td>
                                            @endif
                                            <td class="table-action">
                                                <button wire:click="editData({{ $folder->id }})" data-bs-toggle="modal" data-bs-target="#updateCreateModal" class="action-ico btn-sm btn btn-outline-success mx-1">
                        ...
                        <div class="row mt-4">
                            <div class="col-md-12">
                                <div class="btn-group float-end">
                                    {{ $folders->links('vendor.livewire.bootstrap') }}
                                </div>
                            </div>
                        </div>
                    </div> <!-- end tab-content-->
    ...
    @include('livewire.documents.settings.inc.new-document-folder-form')
    @push('scripts')
            <script>
