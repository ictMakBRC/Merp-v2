    ...
    <div class="row" x-data="{ filter_data: @entangle('filter'), create_new: @entangle('createNew') }">
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    @include('livewire.inventory.stock.inc.new-stock_doc-form')
                </div>               
                <div class="card-body">
                    <div class="tab-content">
                        <div class="row mb-0" @if (!$filter) hidden @endif>
                            <h6>Filter stock_docs</h6>
                        ...
                        <div class="table-responsive">
                            <table id="datableButton" class="table table-striped mb-0 w-100 sortable">
                                ...
                                <tbody>
                                    @foreach ($stock_docs as $key => $stock_doc)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $stock_doc->createdBy->name }}</td>
                                            <td>{{ $stock_doc->unitable->name }}</td>
                                            <td>{{ $stock_doc->stock_code }}</td>
                                            <td>{{ $stock_doc->created_at ?? 'N/A' }}</td>
                                            <td><x-status-badge :status="$stock_doc->status" /></td>
                                            <td class="table-action">
                                                {{-- @livewire('fms.partials.status-component', ['model' => $account, 'field' => 'is_active'], key($account->id)) --}}
                                                @if ($stock_doc->status =='Pending' || $stock_doc->status =='Rejected')
                                                <a href="{{ URL::signedRoute('inventory-stock_doc_items', $stock_doc->stock_code) }}"
                        ...
                        <div class="row mt-4">
                            <div class="col-md-12">
                                <div class="btn-group float-end">
                                    {{ $stock_docs->links('vendor.pagination.bootstrap-5') }}
                                </div>
                            </div>
                        </div>
                    </div> <!-- end tab-content-->
