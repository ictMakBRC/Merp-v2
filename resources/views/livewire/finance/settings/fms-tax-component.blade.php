<div>
                        ...
                        <div class="col-sm-12 mt-3">
                            <div class="d-sm-flex align-items-center">
                                <h5 class="mb-2 mb-sm-0">
                                    @if (!$toggleForm)
                                        Finacial taxs(<span class="text-danger fw-bold">{{ $taxes->total() }}</span>)
                            ...
                            <table id="datableButton" class="table table-striped mb-0 w-100 sortable">
                                ...
                                <tbody>
                                    @foreach ($taxes as $key => $tax)
                                        <tr @if ($tax->is_budget_tax == 1) class="text-success" @endif>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $tax->name }}</td>
                                            <td>{{ $tax->rate ?? 'N/A' }}</td>
                                            @if ($tax->is_active == 0)
                                            ...
                                            <td class="table-action">
                                                <button wire:click="editData({{ $tax->id }})" data-bs-toggle="modal" data-bs-target="#updateCreateModal" class="action-ico btn-sm btn btn-outline-success mx-1">
                        ...
                        <div class="row mt-4">
                            <div class="col-md-12">
                                <div class="btn-group float-end">
                                    {{ $taxes->links('vendor.livewire.bootstrap') }}
                                </div>
                            </div>
                        </div>
                    </div> <!-- end tab-content-->
    ...
    @include('livewire.finance.settings.inc.new-financial-tax-form')
    @push('scripts')
            <script>
