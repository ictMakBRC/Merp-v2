<div>
                        ...
                        <div class="col-sm-12 mt-3">
                            <div class="d-sm-flex align-items-center">
                                <h5 class="mb-2 mb-sm-0">
                                    @if (!$toggleForm)
                                        Chart of Account Subtypes (<span class="text-danger fw-bold">{{ $accountSubTypes->total() }}</span>)
                    ...
                    <div class="tab-content">
                        ...
                        <div class="table-responsive">
                            <table id="datableButton" class="table table-striped mb-0 w-100 sortable">
                                ...
                                <tbody>
                                    @foreach ($accountSubTypes as $key => $accountType)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $accountType->name }}</td>
                                            <td>{{ $accountType->type->name??'N/A' }}</td>
                                            <td>{{ $accountType->description ?? 'N/A' }}</td>
                                            @if ($accountType->is_active == 0)
                                                <td><span class="badge bg-danger">Suspended</span></td>
                                            @else
                                                <td><span class="badge bg-success">Active</span></td>
                                            @endif
                                            <td>{{ date('d-m-Y', strtotime($accountType->created_at)) }}</td>
                                            <td class="table-action">
                                                <button wire:click="editData({{ $accountType->id }})" data-bs-toggle="modal" data-bs-target="#updateCreateModal" class="action-ico btn-sm btn btn-outline-success mx-1">
                        ...
                        <div class="row mt-4">
                            <div class="col-md-12">
                                <div class="btn-group float-end">
                                    {{ $accountSubTypes->links('vendor.livewire.bootstrap') }}
                                </div>
                            </div>
                        </div>
                    </div> <!-- end tab-content-->
    ...
    @include('livewire.finance.settings.inc.new-account-sub-type-form')
    @push('scripts')
            <script>
