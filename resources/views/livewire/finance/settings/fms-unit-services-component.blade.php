<div>
                        ...
                        <div class="col-sm-12 mt-3">
                            <div class="d-sm-flex align-items-center">
                                <h5 class="mb-2 mb-sm-0">
                                    @if (!$toggleForm)
                                        Unit services (<span class="text-danger fw-bold">{{ $services->total() }}</span>)
                    ...
                    <div class="tab-content">
                        ...
                        <div class="table-responsive">
                            <table id="datableButton" class="table table-striped mb-0 w-100 sortable">
                                ...
                                <tbody>
                                    @foreach ($services as $key => $uservice)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $uservice->unitable->name??'NA' }}</td>
                                            <td>{{ $uservice->service->name??'NA' }}</td>
                                            <td>{{ $uservice->service->sku ?? 'N/A' }}</td>
                                            <td>{{ $uservice->sale_price ?? 'N/A' }}</td>
                                            @if ($uservice->is_active == 0)
                                                <td><span class="badge bg-danger">Suspended</span></td>
                                            @else
                                                <td><span class="badge bg-success">Active</span></td>
                                            @endif
                                            <td>{{ date('d-m-Y', strtotime($uservice->created_at)) }}</td>
                                            <td class="table-action">
                                                <button  wire:click="editData({{ $uservice->id }})" data-bs-toggle="modal" data-bs-target="#updateCreateModal" class="action-ico btn-sm btn btn-outline-success mx-1">
    ...
    @include('livewire.finance.settings.inc.new-unit-service-form')
    @push('scripts')
            <script>
