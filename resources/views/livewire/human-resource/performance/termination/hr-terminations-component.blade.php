    ...
    <div class="row" x-data="{ filter_data: @entangle('filter'), create_new: @entangle('createNew') }">
        <div class="col-12">
            <div class="card">
                        ...
                        <div class="col-sm-12 mt-3">
                            <div class="d-sm-flex align-items-center">
                                <h5 class="mb-2 mb-sm-0">
                                    Employee terminations (<span
                                        class="text-danger fw-bold">{{ $terminations->total() }}</span>)
                ...
                @if (Auth::user()->hasPermission(['create_termination'])) 
                <div class="card-body">
                    @include('livewire.human-resource.performance.termination.inc.create-termination-form')
                </div>
                @endif
                <div class="card-body">
                    <div class="tab-content">
                        <div class="row mb-0" @if (!$filter) hidden @endif>
                            <h6>Filter termination</h6>
                            ...
                            <table id="datableButton" class="table table-striped mb-0 w-100 sortable">
                                ...
                                <tbody>
                                    @foreach ($terminations as $key => $termination)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $termination->employee->fullName ?? 'N/A' }}</td>
                                            <td>{{ $termination->subject }}</td>
                                            <td>{{ $termination->employee?->department?->name ?? 'N/A' }}</td>
                                            ...
                                            <td class="table-action">
                                                <a  href="{{ route('hr_termination_view',$termination->id) }}" class="btn btn-sm btn-outline-primary">
                                                    <i class="fa fa-eye"></i>
                                                </a>
