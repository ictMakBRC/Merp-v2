    ...
    <div class="row" x-data="{ filter_data: @entangle('filter'), create_new: @entangle('createNew') }">
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    @include('livewire.human-resource.grievances.inc.new-grievance-form')
                </div>
                <div class="card-body">
                                    ...
                                    @foreach ($grievances as $key => $grievance)
                                        <tr>
                                            ...
                                            <td class="table-action">
                                                ...
                                                @if ($grievance->status == 'Pending' || $grievance->status == 'pending')
                                                    <a href="javascript:void(0)" data-bs-toggle="modal"
                                                        data-bs-target="#submit_modal"
                                                        wire:click='editData({{ $grievance->id }})'
                        ...
                        <div class="row mt-4">
                            <div class="col-md-12">
                                <div class="btn-group float-end">
                                    {{ $grievances->links('vendor.livewire.bootstrap') }}
                                </div>
                            </div>
                        </div>
                    </div> <!-- end tab-content-->
            ...
            <div class="modal-content">
                <div class="modal-header bg-primary">
                    <h6 class="modal-title m-0" id="submitModalTitle">
                        @if ($toggleForm) Submit Your Grievance to:
                        @else
                        Submitted to: 
                         @endif
                        {{ $addressee }}
                    ...
                    <button type="button" class="btn-close text-danger" data-bs-dismiss="modal" @if (!$toggleForm) wire:click='close' @endif
                ...
                <div class="modal-footer">
                    <button type="button" @if (!$toggleForm) wire:click='close' @endif class="btn btn-default btn-sm"
                        data-bs-dismiss="modal">{{ __('public.close') }}</button>
                    @if ($toggleForm)
                        <x-button type="click" wire:click="submitGrievance({{ $edit_id }})"
                            class="btn-success btn-sm">Confirm</x-button>
